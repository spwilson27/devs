...
#### 1.2.1 Turn Envelope Schema & Execution Logic
...
- **Malformed Response**: If the agent returns invalid JSON or fails the schema check, the orchestrator MUST trigger a "Formatting Correction" turn (max 2 retries) before escalating to the user.
- **Partial Completions**: In cases where the LLM hits a token limit mid-envelope, the orchestrator MUST implement a "Resume Protocol" that prompts the agent to complete the JSON payload starting from the last valid token.
- **Tool Timeout**: If an MCP tool exceeds the 300s timeout, the observation returned to the agent MUST include a `TIMEOUT_EXCEEDED` status, forcing the agent to reassess its strategy.
...
- **Requirement-Tagged Logic**: Source code blocks and test cases SHOULD include comments or metadata linking them back to specific `REQ-ID`s from the PRD.
- **Standardized Introspection**: Implementation MUST include "Introspection Points"—predetermined locations in the code (e.g., custom events, state snapshots) that the internal MCP server can query.
...
- **Reasoning Persistence**: The `payload.analysis.reasoning_chain` is never discarded. It is stored as a blob in the `agent_logs` table, allowing for post-hoc analysis of "hallucination triggers."
- **Immutable Observations**: Tool outputs (observations) are captured in their raw, unsummarized form. This prevents agents from misinterpreting failed test results during the reflection phase.
...
#### 2.1.2 Data Models (Orchestrator)
```typescript
...
- **Path Sanitization**: All paths are resolved against the project root. Attempts to access `..` or system paths (e.g., `/etc`) result in an immediate `ACCESS_DENIED` observation.
- **Resource Quotas**: `run_shell_monitored` kills processes exceeding 4GB RSS or 100% CPU for > 10 seconds.
- **Secret Redaction**: Every byte emitted to `stdout` is piped through the `SecretMasker` before being returned to the agent.
...
- **Agent-Oriented Documentation (AOD)**: The ProjectServer MUST register all tools in the `.agent/index.agent.md` manifest so agents know which "Introspection Points" are available.
- **Lifecycle Sync**: The ProjectServer MUST automatically start when the `DeveloperAgent` runs the application's "Dev Mode" within the sandbox.
...
- **`PlanNode`**: The Developer Agent ingests the current task description, relevant PRD/TAS sections, and Medium-term Memory (Epic context). It generates a `TaskStrategy` object.
- **`TestNode` (Red Phase)**: The agent writes a unit or integration test that exercises the target requirement.
- **`CodeNode` (Green Phase)**: The agent modifies the application source code to satisfy the failing test.
- **`RefactorNode`**: The agent performs code cleanup and ensures compliance with project-wide quality standards (linting, types).
- **`ReviewNode` (Independent Validation)**: A separate Reviewer Agent instance validates the work, checking for TAS violations and regression failures.
- **`CommitNode`**: Upon successful review, the state machine triggers an atomic Git commit and updates the SQLite task status.
...
- **turn_index**: Tracked in `agent_logs` to prevent context bloat.
- **Context Refresh**: After 5 turns in a single node, the orchestrator re-injects the TAS and PRD summary to prevent reasoning drift.

#### 3.2.2 The Reviewer Agent's Hierarchy of Concerns
...
- **Retry Protocol**: On a non-zero exit code during the GREEN or VERIFY phases, the orchestrator automatically retries the command twice.
- **Heuristic Analysis**: If the test fails with different errors each time, it is flagged as `FLAKY` and the system pauses for human intervention.
...
#### 3.4.2 State Introspection during Failures
...
#### 3.5.1 The "Shadow Requirement" Problem
...
- **Git-SQLite Synchronization**: Every task completion in the `ImplementationLoop` results in an atomic Git commit. The `tasks` table in `state.sqlite` records the `git_commit_hash` associated with the success state.
- **Rewind Execution Logic**:
...
- **Learning Injection**: The results of the RCA are stored in the **Long-term Memory** (LanceDB) to prevent similar failures in future tasks or projects.
...
- **Task-to-Task Handoff**: Upon completion of a task, the **Reviewer Agent** generates a `TaskSummary` containing:
...
- **ACID Persistence**: Every state transition MUST be recorded in `state.sqlite` before the next tool call is executed.
- **Headless First**: The OrchestratorServer MUST be accessible via standard IPC (Unix Sockets / Named Pipes) to ensure CLI and Extension parity.
- **Deterministic Hashing**: The Entropy Detector MUST use a stable hashing algorithm for error outputs to ensure reliability across different environments.
